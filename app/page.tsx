...
import { ArrowUpRight, BarChart3, Bot, Building2, Check, ChevronDown, CircleHelp, Clock3, Inbox, LayoutDashboard, Mail, Megaphone, MoreHorizontal, Plus, Search, Send, Settings, Sparkles, UsersRound } from 'lucide-react';
...
export default function Home() {
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="main-nav" aria-label="Основная навигация"><p className="nav-label">РАБОЧЕЕ ПРОСТРАНСТВО</p>{nav.map((item) => <button className={`nav-item ${item.active ? 'active' : ''}`} key={item.label}><item.icon size={19} strokeWidth={1.8}/><span>{item.label}</span>{item.count && <span className="nav-count">{item.count}</span>}</button>)}</nav>
