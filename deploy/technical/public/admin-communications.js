(function(){
  ...
  show=function(x){
    ...
    document.querySelector('#chat').innerHTML=`<div class="chathead"><div class="avatar">${esc(initials(client.name))}</div><div><strong>${esc(client.name||'Без имени')}</strong><small>${esc(contactText(client)||'Контакт не указан')} · ${esc(client.source)}</small></div><button class="edit-name-btn" id="editClientBtn" type="button" title="Изменить данные клиента">✎</button><div class="contact-actions">${actionLinks(client)}</div><select class="status" id="chatstatus">${['Новый','В работе','Ожидает','Завершён','Отказ'].map(s=>`<option ${s===client.status?'selected':''}>${s}</option>`).join('')}</select></div><div class="edit-form" id="editForm" hidden><input id="editName" placeholder="Имя" value="${esc(client.name||'')}"><input id="editPhone" placeholder="Телефон" value="${esc(client.phone||'')}"><input id="editEmail" placeholder="E-mail" value="${esc(client.email||'')}"><input id="editTelegram" placeholder="Telegram" value="${esc(client.telegram||'')}"><button class="save" id="saveClientBtn" type="button">Сохранить</button><button class="cancel" id="cancelEditBtn" type="button">Отмена</button></div><div class="conversation"><div class="notice"><b>🤖 Бот ведёт переговоры</b><span>Менеджер может подключиться к диалогу</span></div><div class="day">История переговоров · ${conversation.length} сообщений клиента</div>${bubbles||'<div class="placeholder">Сообщений пока нет</div>'}</div><div class="message-result" id="messageResult"></div><div class="manager-composer"><input id="managerMessage" maxlength="2000" placeholder="${canMessage?'Написать клиенту от имени менеджера…':'Для сообщения по телефону подключите WhatsApp или SMS'}" ${canMessage?'':'disabled'}><button class="send" id="sendManager" ${canMessage?'':'disabled'}>➤</button></div><div class="notearea">Заметка менеджера: ${esc(client.note||'не добавлена')}</div><div class="composer"><input id="noteinput" value="${esc(client.note||'')}" placeholder="Добавить внутреннюю заметку…"><button class="send" id="savenote">✓</button></div>`;
    ...
    document.querySelector('#editClientBtn').onclick=()=>{document.querySelector('#editForm').hidden=false};
    document.querySelector('#cancelEditBtn').onclick=()=>{document.querySelector('#editForm').hidden=true};
    document.querySelector('#saveClientBtn').onclick=()=>patch(client.id,{name:document.querySelector('#editName').value,phone:document.querySelector('#editPhone').value,email:document.querySelector('#editEmail').value,telegram:document.querySelector('#editTelegram').value});
    if(canMessage){
